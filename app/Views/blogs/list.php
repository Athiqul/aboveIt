      ...
      <div class="row">
            ...
            <!-- News Block -->
             ...
             <?php foreach($blogs->msg as $item):?>
              ...
              <div class="inner-box">
                <div class="image-box">
                  <figure class="image">
                    <a href="<?=site_url('/blogs/'.$item->id.'?blog='.$item->title)?>"
                      ><img src="<?=getenv('IMAGE_LINK').'blog-image/'.$item->image?>" alt="<?=$item->title?>"
                    /></a>
                  </figure>
                </div>
                <div class="content-box">
                  <span class="date"><?= date('j M Y',strtotime($item->publish_at))?></span>
                  ...
                  <a href="<?=site_url('/blogs/'.$item->id.'?blog='.$item->title)?>" class="read-more"
          ...
          <div class="row">
            <ul class="pagination">
              ...
              ?>
              <li class="page__btn">
                <a href="<?=site_url('/blogs?page=1')?>">
                <i class="fa-solid fa-angles-left"></i>
                </a>
               
              </li>
              <?php for($i=1;$i<=$blogs->totalPage;$i++):?>
              <li class="page__numbers <?=($i==$current)?'text-danger':''?>"><a class="<?=($i==$current)?'text-danger':''?>" href="<?=site_url('/blogs?page=').$i?>"><?=$i?></a></li>
              <?php endfor?>
             
              
              <li class="page__btn">
               <a href="<?=site_url('/blogs?page=').$blogs->totalPage?>">
